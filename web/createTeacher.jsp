...
<html>
    <head>
        <title>ERP - Beta Version | New Teacher Registration</title>
        <meta charset="UTF-8">
		...
		*
		...
		function checkId()
		{
			...
			if(isNaN(tid) && (tid.indexOf('CU') >= 3 || tid.indexOf('CU') == -1))
			{
				document.getElementById("output").innerHTML = "*Invalid Teacher ID";
				document.getElementById('submit').style.visibility = 'hidden';
			}
			else
			{
	...
	ArrayList<Courses> cl = cdao.getCourses();
	%>
        <div style = "background-color:white;padding: 0px 20px 20px 20px;">
			<form id = "new-student-form" action = "addTeacher" method="POST">
				<fieldset>
					<legend align="center">
					...
					<table>
						<tr>
							...
							<td>
								<input type="text" placeholder="Name" name = "tname" id = "tname" autofocus required>
							</td>
						</tr>
						...
						<tr>
							...
							<td>
								<input type="text" placeholder="ID" name = "tid" id = "tid" onchange = "checkId()" required>
							</td>
						</tr>
						...
						<tr>
							...
							<td>
								<select type="select" id = "coursecode" name = "coursecode" required>
									<option value=""> Select Course Code </option>
									<%
									for(Courses c : cl)
									{
										%>
										<option value = "<%=c.getCoursecode()%>"> <%=c.getCoursecode()%> - <%=c.getTitle()%></option>
										<%
									}
									%>
								</select>
